 ...
 * valor de los viajes activos de cada cruce, el total de cada transportista y la
 * suma de todas las pólizas. Se imprime en horizontal y, cuando hay más pólizas
 * de las que caben, se reparten en varias hojas (cada una con su propia suma).
 */
...
export default function ReportePolizasTransportistasPage() {
  ...
  const exportar = () => exportarExcel('Pólizas Activas por Transportista', [
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Valor de los viajes activos de cada transportista en cada póliza abierta."
      />
      ...
      <div className="toolbar" style={{ alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        ...
        {data && (
          <div style={{ marginLeft: 'auto', fontSize: 13, color: '#6b7280' }}>
            ...
            <b>Total: {formatCurrency(data.total_general)}</b>
          </div>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="card"><div className="card-body" style={{ color: '#6b7280' }}>
          No hay viajes activos en pólizas abiertas.
        </div></div>
      ) : (
                  ...
                  {data.polizas.map((p) => {
                    ...
                    return (
                      <td key={p.codigo} style={{ textAlign: 'right' }}>
                        {v ? formatNumber(v) : <span style={{ color: '#d1d5db' }}>—</span>}
                      </td>
                    );
                  })}
                  <td style={{ textAlign: 'right', fontWeight: 700 }}>{formatNumber(f.total)}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr style={{ fontWeight: 700 }}>
                <td />
                <td>Total por póliza</td>
                {data.polizas.map((p) => (
                  <td key={p.codigo} style={{ textAlign: 'right' }}>
                    {formatNumber(data.totales_por_poliza?.[p.codigo] || 0)}
                  </td>
                ))}
